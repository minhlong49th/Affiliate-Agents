...
name: lp-qa-checker
description: |
  Worker 4 (QA) in the LP builder pipeline. Scores the content blueprint
  against factual accuracy and framework compliance rubrics. Called by
...
Read `./output/[brand_slug]-[start_running_time]/.content_blueprint.json` — blueprint to score.
Read `./output/[brand_slug]-[start_running_time]/.pipeline_input.json` — for lp_type, attempt_number, and brand_slug.
...
- If `attempt_number = 3` → set `pass_to_worker_3 = true` regardless (force-pass)
  and add to summary: "FORCE-PASSED — unresolved issues require manual review"
